...
ChatService::ChatService(ThreadSafePtr<PubNub> pubnub) : pubnub(pubnub) {}

void ChatService::init_services(const ChatConfig& config) {
    std::cout << "Initializing chat services...2" << std::endl;
    channel_service =
        std::make_shared<ChannelService>(pubnub, weak_from_this(), config.rate_limit_factor);
    user_service = std::make_shared<UserService>(
        pubnub,
        weak_from_this(),
        config.store_user_activity_interval
    );
    ...
    std::cout << "Initializing chat services...2.5" << std::endl;
    this->callback_service =
        std::make_shared<CallbackService>(service_bundle, presence_service, pubnub);

    std::cout << "Initializing chat services...3" << std::endl;
    this->pubnub->lock()->set_logging_callback(Logger::log_ccore_message);
}

ThreadSafePtr<PubNub> ChatService::create_pubnub(
    const String& publish_key,
    const String& subscribe_key,
...
void ChatService::emit_chat_event(
    ...
    String event_message = String("{") + payload_parameters + String(", \"type\": \"")
        + chat_event_type_to_string(chat_event_type) + String("\"}");
    ...
    event_method == EventMethod::Signal ? pubnub_handle->signal(channel_id, event_message)
                                        : pubnub_handle->publish(channel_id, event_message);
}

std::tuple<std::vector<Pubnub::Event>, bool> ChatService::get_events_history(
    ...
    if (!messages_array_json.contains(channel_id)) {
        std::tuple<std::vector<Pubnub::Event>, bool> return_tuple =
            std::make_tuple(events, is_more);
        return return_tuple;
    }
    for (auto element : messages_array_json[channel_id]) {
        ...
        event.type =
            chat_event_type_from_string(message_json.get_string("type").value_or(String("")));
...
std::tuple<std::vector<Pubnub::UserMentionData>, bool> ChatService::get_current_user_mentions(
    const Pubnub::String& start_timetoken,
    const Pubnub::String& end_timetoken,
    int count
) const {
    ...
    auto events_history =
        this->get_events_history(current_user, start_timetoken, end_timetoken, count);
        ...
        [this](const Event& event) {
            ...
            auto messages = [this, previous_timetoken, timetoken, channel_id] {
                ...
                return pubnub_handle
                    ->fetch_history(channel_id.value(), previous_timetoken, timetoken.value(), 1);
            }();
...
std::shared_ptr<Subscription> ChatService::listen_for_events(
    const Pubnub::String& channel_id,
    Pubnub::pubnub_chat_event_type chat_event_type,
    std::function<void(const Pubnub::Event&)> event_callback
) const {
    if (channel_id.empty()) {
    ...
    subscription->add_event_listener(
        this->callback_service->to_c_event_callback(chat_event_type, event_callback),
        chat_event_type
    );
...
void ChatService::register_logger_callback(
