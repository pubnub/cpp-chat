...
void CallbackService::register_message_callback(
...
void CallbackService::register_thread_message_update_callback(
    Pubnub::String channel_id,
    std::function<void(Pubnub::ThreadMessage)> thread_message_callback
) {
    this->callbacks.get_thread_message_update_callbacks().update_or_insert(
        channel_id,
        thread_message_callback
    );
}
...
void CallbackService::register_message_update_callback(
    ...
    this->callbacks.get_message_update_callbacks().update_or_insert(
        message_timetoken,
        message_update_callback
    );
}
...
void CallbackService::register_channel_callback(
    Pubnub::String channel_id,
    std::function<void(Pubnub::Channel)> channel_callback
) {
...
void CallbackService::register_event_callback(
    ...
    this->callbacks.get_event_callbacks().update_or_insert(
        channel_id,
        std::make_pair(chat_event_type, event_callback)
    );
}

void CallbackService::remove_event_callback(
    Pubnub::String channel_id,
    Pubnub::pubnub_chat_event_type chat_event_type
) {
...
void CallbackService::register_user_callback(
    Pubnub::String user_id,
    std::function<void(Pubnub::User)> user_callback
) {
...
void CallbackService::register_channel_presence_callback(
    Pubnub::String channel_id,
    std::function<void(std::vector<Pubnub::String>)> presence_callback
) {
    this->callbacks.get_channel_presence_callbacks().update_or_insert(
        channel_id,
        presence_callback
    );
}
...
void CallbackService::register_membership_callback(
    ...
    this->callbacks.get_membership_callbacks().update_or_insert(
        channel_id,
        std::make_pair(user_id, membership_callback)
    );
}
...
void CallbackService::broadcast_callbacks_from_message(pubnub_v2_message message) {
    ...
    Pubnub::String message_channel_string =
        Pubnub::String(message.channel.ptr, message.channel.size);
    ...
    if (Parsers::PubnubJson::is_presence(message_string)) {
        ...
        auto maybe_callback =
            this->callbacks.get_channel_presence_callbacks().get(normal_channel_name);
        if (maybe_callback.has_value()) {
    ...
    if (Parsers::PubnubJson::is_message_update(message_string)) {
        Pubnub::String message_timetoken =
            Parsers::PubnubJson::message_update_timetoken(message_string);

        auto parsed_message = this->message_service.lock()->create_message_object(
            Parsers::PubnubJson::to_message_update(message)
        );

        auto prefix_length = std::strlen(Pubnub::MESSAGE_THREAD_ID_PREFIX) - 1;
        if (message.channel.size >= prefix_length
            && std::strncmp(message.channel.ptr, Pubnub::MESSAGE_THREAD_ID_PREFIX, prefix_length)
                == 0) {
            auto maybe_callback =
                this->callbacks.get_thread_message_update_callbacks().get(message_timetoken);
            if (maybe_callback.has_value()) {
                ...
                auto parent_channel =
                    channel.substring(prefix_length, channel.length() - channel_padding_length);

                maybe_callback.value()(Pubnub::ThreadMessage(parsed_message, parent_channel));
            }
        } else {
            auto maybe_callback =
                this->callbacks.get_message_update_callbacks().get(message_timetoken);
            if (maybe_callback.has_value()) {
            ...
            if (user_from_message == membership_user) {
                ...
                if (memberships && channels && users) {
                    ...
                    throw std::runtime_error(
                        "Membership, channel or user service is not available to call callback"
                    );
                }
            }
        }
...
CCoreCallbackData CallbackService::to_c_message_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_message(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                ...
                if (auto service = ctx.chat_service.lock()) {
                    ctx.message_callback(
                        service->message_service->create_message_object(parsed_message)
                    );
                } else {
...
CCoreCallbackData CallbackService::to_c_channel_update_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_channel_update(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));
                auto channel = ctx.channel_service->create_channel_object(
                    Parsers::PubnubJson::to_channel(message)
                );
                auto updated =
                    ctx.channel_service->update_channel_with_base(channel, ctx.base_channel);
                ctx.channel_update_callback(updated);
            }
        },
        this->callback_contexts.emplace_back(
            CallbackCtx {base_channel, channel_service, channel_update_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_channels_updates_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_channel_update(
                ...
                ChannelEntity stream_channel_entity =
                    ChannelDAO(stream_channel->channel_data()).to_entity();
                ChannelEntity channel_entity = ChannelDAO(channel.channel_data()).to_entity();
                auto pair = std::make_pair(
                    channel.channel_id(),
                    ChannelEntity::from_base_and_updated_channel(
                        stream_channel_entity,
                        channel_entity
                    )
                );
                ...
                std::transform(
                    ...
                    [&channel, updated_channel](const Pubnub::Channel& base_channel) {
                        return base_channel.channel_id() == channel.channel_id() ? updated_channel
                                                                                 : base_channel;
                    }
                );
        ...
        this->callback_contexts.emplace_back(
            CallbackCtx {channels, channel_service, channel_update_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_user_update_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_user_update(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));
                auto user =
                    ctx.user_service->create_user_object(Parsers::PubnubJson::to_user(message));
                auto updated = ctx.user_service->update_user_with_base(user, ctx.user_base);
                ctx.user_update_callback(updated);
            }
        },
        this->callback_contexts.emplace_back(
            CallbackCtx {user_base, user_service, user_update_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_users_updates_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_user_update(
                ...
                auto updated_user = ctx.user_service->create_user_object(
                    {stream_user->user_id(),
                     UserEntity::from_base_and_updated_user(stream_user_entity, user_entity)}
                );
                ...
                std::transform(
                    ctx.users.begin(),
                    ctx.users.end(),
                    std::back_inserter(updated_users),
                    [&user, updated_user](const Pubnub::User& base_user) {
...
CCoreCallbackData CallbackService::to_c_event_callback(
    Pubnub::pubnub_chat_event_type chat_event_type,
    std::function<void(Pubnub::Event)> event_callback
) {
    struct CallbackCtx {
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_event(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));
                auto parsed_event = Parsers::PubnubJson::to_event(message);
...
CCoreCallbackData CallbackService::to_c_presence_callback(
    Pubnub::String channel_id,
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_presence(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));

                Pubnub::String normal_channel_name =
                    Pubnub::String(message.channel.ptr, message.channel.size);
                auto pnpres_length = strlen("-pnpres");
        ...
        this->callback_contexts.emplace_back(
            CallbackCtx {channel_id, presence_service, presence_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_membership_update_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_membership_update(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));

                auto membership_channel = Parsers::PubnubJson::membership_channel(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                );
                auto membership_user = Parsers::PubnubJson::membership_user(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                );
                ...
                if (membership_channel == channel_id && membership_user == user_id) {
                    auto custom_field = Parsers::PubnubJson::membership_from_string(
                        Pubnub::String(message.payload.ptr, message.payload.size)
                    );
                    ...
                    Pubnub::Membership membership_obj =
                        chat->membership_service->create_membership_object(
                            chat->user_service->get_user(user_id),
                            chat->channel_service->get_channel(channel_id),
                            custom_field
                        );
                    chat->membership_service->update_membership_with_base(
                        membership_obj,
                        ctx.membership_base
                    );
        ...
        this->callback_contexts.emplace_back(
            CallbackCtx {membership_base, chat_service, membership_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_memberships_updates_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_membership_update(
                ...
                auto stream_membership = std::find_if(
                    ...
                    ctx.memberships.end(),
                    [&membership_channel,
                     &membership_user](const Pubnub::Membership& base_membership) {
                ...
                if (stream_membership != ctx.memberships.end()) {
                    ...
                    MembershipEntity stream_membership_entity =
                        MembershipDAO(stream_membership->custom_data()).to_entity();
                    MembershipEntity membership_entity =
                        MembershipDAO(membership_obj.custom_data()).to_entity();
                    auto updated_membership = chat->membership_service->create_membership_object(
                        stream_membership->user,
                        stream_membership->channel,
                        MembershipEntity::from_base_and_updated_membership(
                            stream_membership_entity,
                            membership_entity
                        )
                    );
                    ...
                    std::transform(
                        ...
                        [&membership_channel,
                         &membership_user,
                         updated_membership](const Pubnub::Membership& base_membership) {
                            return base_membership.channel.channel_id() == membership_channel
                                    && base_membership.user.user_id() == membership_user
                                ? updated_membership
                                : base_membership;
        ...
        this->callback_contexts.emplace_back(
            CallbackCtx {memberships, chat_service, membership_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_message_update_callback(
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_message_update(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));

                auto message_timetoken = Parsers::PubnubJson::message_update_timetoken(
                    Pubnub::String(message.payload.ptr, message.payload.size)
                );
                if (message_timetoken == ctx.base_message.timetoken()) {
                    auto parsed_message = ctx.message_service->create_message_object(
                        Parsers::PubnubJson::to_message_update(message)
                    );
                    auto updated_message = ctx.message_service->update_message_with_base(
                        parsed_message,
                        ctx.base_message
                    );
        ...
        this->callback_contexts.emplace_back(
            CallbackCtx {base_message, message_service, message_update_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_thread_message_update_callback(
        ...
        +[](const pubnub_t* pb, struct pubnub_v2_message message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_message_update(
                ...
                if (message_timetoken == ctx.base_message.timetoken()) {
                    ...
                    ctx.message_update_callback(
                        Pubnub::ThreadMessage(updated_message, ctx.base_message.parent_channel_id())
                    );
                }
            }
        },
...
CCoreCallbackData CallbackService::to_c_messages_updates_callback(
    const std::vector<Pubnub::Message>& messages,
    std::shared_ptr<const MessageService> message_service,
    ...
    return {
        +[](const pubnub_t* pb, struct pubnub_v2_message pn_message, void* ctx_ptr) {
            if (Parsers::PubnubJson::is_message_update(
                    Pubnub::String(pn_message.payload.ptr, pn_message.payload.size)
                )) {
                auto ctx = std::any_cast<CallbackCtx>(*reinterpret_cast<std::any*>(ctx_ptr));
                ...
                if (message != ctx.messages.end()) {
                    auto parsed_message = ctx.message_service->create_message_object(
                        Parsers::PubnubJson::to_message_update(pn_message)
                    );
                    auto updated_message =
                        ctx.message_service->update_message_with_base(parsed_message, *message);
                    ...
                    std::transform(
                        ...
                        [&message_timetoken, updated_message](const Pubnub::Message& base_message) {
                            return base_message.timetoken() == message_timetoken ? updated_message
                                                                                 : base_message;
                        }
                    );
        ...
        this->callback_contexts.emplace_back(
            CallbackCtx {messages, message_service, message_update_callback}
        )
    };
}

CCoreCallbackData CallbackService::to_c_thread_messages_updates_callback(
            ...
            if (Parsers::PubnubJson::is_message_update(
                ...
                if (message != ctx.messages.end()) {
                    ...
                    std::transform(
                        ...
                        std::back_inserter(updated_messages),
                        [&message_timetoken,
                         updated_message](const Pubnub::ThreadMessage& base_message) {
                            return base_message.timetoken() == message_timetoken
                                ? Pubnub::ThreadMessage(
                                      updated_message,
