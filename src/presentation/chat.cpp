...
Chat::Chat(
    ...
    std::cout << "Initializing chat services..." << std::endl;
...
Chat Chat::init(
    ...
    std::cout << "Initializing chat...2" << std::endl;
//    try {
...
//    }
    std::cout << "Initializing chat...3" << std::endl;
    if (config.store_user_activity_timestamps) {
        chat.store_user_activity_timestamp();
    }
    std::cout << "Initializing chat...4" << std::endl;
    return chat;
}

Channel Chat::create_public_conversation(
    const String& channel_id,
    const ChatChannelData& channel_data
) const {
    return this->channel_service->create_public_conversation(channel_id, channel_data);
}

CreatedChannelWrapper Chat::create_direct_conversation(
    const User& user,
    const String& channel_id,
    const ChatChannelData& channel_data,
    const String& membership_data
) const {
    auto return_tuple =
        this->channel_service
            ->create_direct_conversation(user, channel_id, channel_data, membership_data);
    return CreatedChannelWrapper(
        std::get<0>(return_tuple),
        std::get<1>(return_tuple),
        Pubnub::Vector<Membership>(std::move(std::get<2>(return_tuple)))
    );
}

CreatedChannelWrapper Chat::create_group_conversation(
    Pubnub::Vector<User> users,
    const String& channel_id,
    const ChatChannelData& channel_data,
    const String& membership_data
) const {
    auto return_tuple = this->channel_service->create_group_conversation(
        users.into_std_vector(),
        channel_id,
        channel_data,
        membership_data
    );
...
ChannelsResponseWrapper Chat::get_channels(
    ...
    return ChannelsResponseWrapper(
        {Pubnub::Vector<Channel>(std::move(std::get<0>(return_tuple))),
         std::get<1>(return_tuple),
         std::get<2>(return_tuple)}
    );
}
...
Pubnub::Vector<Channel> Pubnub::Chat::get_channel_suggestions(String text, int limit) const {
    return Pubnub::Vector<Channel>(
        std::move(this->channel_service->get_channel_suggestions(text, limit))
    );
}
...
UsersResponseWrapper Chat::get_users(
    const Pubnub::String& filter,
    const Pubnub::String& sort,
    int limit,
    const Pubnub::Page& page
    ...
    return UsersResponseWrapper(
        {Pubnub::Vector<User>(std::move(std::get<0>(return_tuple))),
         std::get<1>(return_tuple),
         std::get<2>(return_tuple)}
    );
}
...
void Chat::emit_chat_event(
    pubnub_chat_event_type chat_event_type,
    const String& channel_id,
...
EventsHistoryWrapper Pubnub::Chat::get_events_history(
    const Pubnub::String& channel_id,
    const Pubnub::String& start_timetoken,
    const Pubnub::String& end_timetoken,
    int count
) const {
    auto return_tuple =
        this->chat_service->get_events_history(channel_id, start_timetoken, end_timetoken, count);
    return EventsHistoryWrapper(
        {Pubnub::Vector<Event>(std::move(std::get<0>(return_tuple))), std::get<1>(return_tuple)}
    );
}

CallbackHandle Chat::listen_for_events(
    const String& channel_id,
    pubnub_chat_event_type chat_event_type,
...
Pubnub::Vector<UnreadMessageWrapper> Chat::get_unread_messages_counts(
    const Pubnub::String& filter,
    const Pubnub::String& sort,
    int limit,
    const Pubnub::Page& page
    ...
    auto tuples =
        this->membership_service->get_all_unread_messages_counts(filter, sort, limit, page);
...
MarkMessagesAsReadWrapper Pubnub::Chat::mark_all_messages_as_read(
    const Pubnub::String& filter,
    const Pubnub::String& sort,
    int limit,
    const Pubnub::Page& page
    ...
    auto return_tuple =
        this->membership_service->mark_all_messages_as_read(filter, sort, limit, page);
...
UserMentionDataList Pubnub::Chat::get_current_user_mentions(
    const Pubnub::String& start_timetoken,
    const Pubnub::String& end_timetoken,
    int count
) const {
    auto return_tuple =
        this->chat_service->get_current_user_mentions(start_timetoken, end_timetoken, count);

    UserMentionDataList return_list;
    return_list.user_mention_data =
        Pubnub::Vector<Pubnub::UserMentionData>(std::move(std::get<0>(return_tuple)));
...
void Pubnub::Chat::register_logger_callback(
    std::function<void(Pubnub::pn_log_level, const char*)> callback
) {
...
Pubnub::User Chat::create_user_for_init_chat(
    const Pubnub::String& user_id,
    const Pubnub::ChatUserData& user_data
) const {
